...
const BASE_URL = 'https://hairsupplystorenearme.com';
...
<head>
  ...
  <title>Sitemap - Hair Supply Store Near Me</title>
...
<body class="bg-gray-50">
  <header class="bg-indigo-900 text-white py-6">
    <div class="container mx-auto px-4">
      ...
      <p class="mt-2">A complete index of all pages on HairSupplyStoreNearMe.com</p>
    </div>
  </header>
  
  <main class="container mx-auto px-4 py-8">
    <div class="mb-10">
      <h2 class="text-2xl font-bold text-indigo-800 mb-4">Main Pages</h2>
      <ul class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
        ...
        <li><a href="/about/" class="text-indigo-600 hover:text-indigo-800">About</a></li>
        <li><a href="/contact/" class="text-indigo-600 hover:text-indigo-800">Contact</a></li>
        <li><a href="/add-a-listing/" class="text-indigo-600 hover:text-indigo-800">Add a Listing</a></li>
        <li><a href="${getAllStatesUrl()}" class="text-indigo-600 hover:text-indigo-800">All States</a></li>
        <li><a href="${getAllCitiesUrl()}" class="text-indigo-600 hover:text-indigo-800">All Cities</a></li>
      </ul>
    </div>
    
    <div class="mb-10">
      <h2 class="text-2xl font-bold text-indigo-800 mb-4">States (${filteredStates.length})</h2>
      <ul class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2">
        ${filteredStates.map(state => `
          <li><a href="${getStateUrl(state.slug || '')}" class="text-indigo-600 hover:text-indigo-800">${state.state}</a></li>
        `).join('')}
      </ul>
    </div>
    
    <div class="mb-10">
      <h2 class="text-2xl font-bold text-indigo-800 mb-4">Categories (${filteredCategories.length})</h2>
      <ul class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2">
        ${filteredCategories.map(category => `
          <li><a href="${getCategoryUrl(category.slug || '')}" class="text-indigo-600 hover:text-indigo-800">${category.category}</a></li>
        `).join('')}
      </ul>
    </div>
    
    <div class="mb-10">
      <h2 class="text-2xl font-bold text-indigo-800 mb-4">Cities (${filteredCities.length})</h2>
      <div class="bg-white p-5 rounded-lg shadow-md">
        ...
        <ul id="cityList" class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2 max-h-96 overflow-y-auto">
          ${filteredCities.map(city => `
            <li data-name="${city.city.toLowerCase()}"><a href="${getCityUrl(city.slug || '')}" class="text-indigo-600 hover:text-indigo-800">${city.city}, ${city.state?.state || ''}</a></li>
          `).join('')}
        </ul>
      </div>
    </div>
    ...
    <div class="mb-10">
      <h2 class="text-2xl font-bold text-indigo-800 mb-4">Hair Supply Store Listings (${filteredSalons.length})</h2>
      <div class="bg-white p-5 rounded-lg shadow-md">
        <div class="mb-4">
          <input type="text" id="salonSearch" placeholder="Search hair supply stores..." 
            class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500">
        </div>
        <ul id="salonList" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-96 overflow-y-auto">
          ${filteredSalons.map(salon => `
            <li data-name="${salon.title.toLowerCase()}"><a href="${getSalonUrl(salon.slug || '')}" class="text-indigo-600 hover:text-indigo-800">${salon.title}</a></li>
          `).join('')}
        </ul>
      </div>
    </div>
  ...
  <footer class="bg-indigo-900 text-white py-6">
    <div class="container mx-auto px-4 text-center">
      <p>&copy; ${new Date().getFullYear()} HairSupplyStoreNearMe.com. All rights reserved.</p>
    </div>
  </footer>
...
export async function generateRobotsTxt(multipleXmlSitemaps: boolean = false): Promise<void> {
  ...
  let robotsTxt = `# robots.txt for HairSupplyStoreNearMe.com
User-agent: *
Allow: /
