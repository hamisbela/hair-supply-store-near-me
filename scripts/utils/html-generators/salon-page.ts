...
export function generateSalonPage(salon: BeautySalon): string {
  ...
  const metaDescription = `${salon.title} is a hair supply store in ${cityName}, ${stateName}. ${salon.description?.substring(0, 150) || 'Visit us for quality hair care products, tools, and accessories.'}`;
  ...
  if (salon.city && salon.city.salons && salon.city.salons.length > 1) {
    ...
    if (nearbySalons.length > 0) {
      ...
      <div class="mt-12 border-t border-gray-200 pt-12">
        <h2 class="text-2xl font-bold mb-6 text-indigo-800">More Hair Supply Stores in ${cityName}</h2>
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
              <a href="${getSalonUrl(nearbySlug)}">
                ${nearbySalon.images && nearbySalon.images.length > 0 
                  ? `<img src="https://img.hairsupplystorenearme.com/your-r2-bucket-name${nearbySalon.images[0].path}" alt="${nearbySalon.title}" class="w-full h-40 object-cover">`
                  : `<div class="w-full h-40 bg-gradient-to-r from-indigo-400 to-purple-500 flex items-center justify-center text-white text-3xl">
              ...
              <div class="p-4">
                <a href="${getSalonUrl(nearbySlug)}" class="block">
                  <h3 class="font-bold text-lg text-indigo-800 hover:text-indigo-600">${nearbySalon.title}</h3>
                </a>
                <p class="text-gray-600 mt-1 text-sm">
                  <i class="fas fa-map-marker-alt text-indigo-500 mr-1"></i> ${cityName}, ${stateName}
                </p>
                ${nearbySalon.telephone ? `
                <p class="mt-3">
                  <a href="tel:${nearbySalon.telephone}" class="inline-block bg-indigo-600 text-white px-3 py-1 rounded text-sm hover:bg-indigo-700">
  ...
  if (salon.latitude && salon.longitude) {
    ...
    mapEmbed = `
    <div class="mb-6">
      <h4 class="font-semibold mb-3 text-indigo-800">Location</h4>
      ...
      <a 
        ...
        class="block w-full bg-indigo-600 text-white text-center py-2 px-4 rounded-lg hover:bg-indigo-700 transition-colors"
      >
  ...
  <main class="container mx-auto px-4 py-6">
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      <div class="relative h-40 md:h-56 bg-indigo-800">
        ${salon.images && salon.images.length > 0 
          ? `<img src="https://img.hairsupplystorenearme.com/your-r2-bucket-name${salon.images[0].path}" alt="${salon.title}" class="w-full h-full object-cover opacity-70">`
          : `<div class="w-full h-full bg-gradient-to-r from-indigo-800 to-purple-800"></div>`
        }
        <div class="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent">
          <div class="container mx-auto">
            <h1 class="text-2xl md:text-3xl font-bold text-white">${salon.title} - Hair Supply in ${cityName}, ${stateName}</h1>
            <div class="flex flex-wrap items-center gap-2 mt-1 text-white/90 text-sm">
              ${salon.categories && salon.categories.length > 0 ? 
                `<div class="flex items-center">
                  <i class="fas fa-tag text-indigo-300 mr-1"></i>
                  ${salon.categories.map(cat => cat.category).join(' • ')}
                </div>` : 
                `<div class="flex items-center">
                  <i class="fas fa-store text-indigo-300 mr-1"></i>
                  Beauty Supply Store
                </div>`
              }
              ...
              ${salon.city ? 
                `<div class="flex items-center ml-2">
                  <i class="fas fa-map-marker-alt text-indigo-300 mr-1"></i>
                  ${cityName}, ${stateName}
                </div>` : ''
              }
              ...
              ${salon.telephone ? 
                `<div class="flex items-center ml-2">
                  <i class="fas fa-phone text-indigo-300 mr-1"></i>
                  ${salon.telephone}
                </div>` : ''
              }
            </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 p-6">
        ...
        <div class="md:col-span-2">
          <div class="mb-8">
            ...
            <div class="bg-gray-50 p-5 rounded-lg border-l-4 border-indigo-500">
              <p class="text-gray-700 leading-relaxed">
                ${salon.description || `${salon.title} offers a wide selection of professional hair care products, styling tools, and accessories in ${cityName}, ${stateName}. Visit us to find quality products for all your hair styling needs.`}
              </p>
            </div>
          </div>
          ...
          <div class="mb-8">
            <h2 class="text-2xl font-bold mb-4 text-indigo-800 flex items-center">
              <i class="fas fa-list-check text-indigo-600 mr-3"></i>
              Products
            </h2>
          ...
          <div class="mb-8">
            ...
            <div class="space-y-4">
              ${salon.reviews_data.map(review => `
                <div class="bg-gray-50 p-5 rounded-lg border-l-4 border-indigo-300">
                  <div class="flex justify-between items-center mb-2">
                    <span class="font-bold text-indigo-900">${review.author || 'Anonymous'}</span>
          ...
          <div class="mb-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
              ${salon.images.slice(1).map(image => 
                `<a href="https://img.hairsupplystorenearme.com/your-r2-bucket-name${image.path}" target="_blank" class="block h-40 overflow-hidden rounded-lg shadow-md">
                  <img src="https://img.hairsupplystorenearme.com/your-r2-bucket-name${image.path}" alt="${salon.title}" class="w-full h-full object-cover hover:opacity-90 transition-opacity">
                </a>`
              ).join('')}
            </div>
          </div>
        ...
        <div>
          <div class="bg-gray-50 rounded-lg p-6 shadow-md sticky top-20">
            <h3 class="text-xl font-bold mb-4 text-indigo-800 border-b pb-2 border-gray-200">
            ...
            <div class="flex mb-4">
              ...
              <div>
                ...
                <a href="tel:${salon.telephone}" class="text-indigo-600 hover:text-indigo-800">${salon.telephone}</a>
              </div>
            </div>
            ` : ''}
            ...
            <div class="flex mb-4">
              ...
              <div>
                ...
                <a href="mailto:${salon.email}" class="text-indigo-600 hover:text-indigo-800">${salon.email}</a>
              </div>
            </div>
            ` : ''}
            ...
            <div class="mb-4">
              <h4 class="font-semibold text-gray-800 mb-3 flex items-center">
                <div class="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center mr-2">
  ...
  return pageWrapper(`${salon.title} - Hair Supply Store in ${cityName}, ${stateName}`, metaDescription, content);
}